...
export const getPricesForVariant = (variant: any, countryCode?: string) => {
  ...
  if (variant.prices && variant.prices.length > 0) {
    // For USD requests, first try to find a USD price
    if (normalizeCurrency(targetCurrency) === 'USD') {
      const usdPrices = variant.prices
        .filter((p: any) => normalizeCurrency(p.currency_code) === 'USD')
        .filter((p: any) => typeof p.amount === 'number' && !isNaN(p.amount));
      
      if (usdPrices.length > 0) {
        // Sort by amount and take the lowest
        const lowestUsdPrice = usdPrices.sort((a: any, b: any) => a.amount - b.amount)[0];
        
        if (debugVariant) console.log(`- Found USD price: ${lowestUsdPrice.amount}`);
        
        const result = {
          calculated_price_number: lowestUsdPrice.amount,
          calculated_price: convertToLocale({
            amount: lowestUsdPrice.amount,
            currency_code: 'USD'
          }),
          original_price_number: lowestUsdPrice.amount,
      ...
      // If no USD price found, try to convert from GBP
      ...
      if (gbpPrices.length > 0) {
        ...
        const convertedAmount = convertCurrency(lowestGbpPrice.amount, 'GBP', 'USD');
        
        if (debugVariant) console.log(`- Converting GBP price ${lowestGbpPrice.amount} to USD: ${convertedAmount}`);
      ...
      if (debugVariant) console.log(`- No USD or GBP price found for conversion, will try other fallbacks`);
    }
